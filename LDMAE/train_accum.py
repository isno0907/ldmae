...
def do_train(train_config, accelerator):
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f'Use QK Normalization: {train_config["model"]["use_qknorm"]}')
    ...
    gradient_accumulation_steps = int(train_config['train']['gradient_accumulation_steps'])
    global_batch_size = batch_size_per_gpu * accelerator.num_processes
    train_config['train']['max_steps'] = train_config['train']['max_steps']
    loader = DataLoader(
        dataset,
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"Batch size {batch_size_per_gpu} per gpu, with {global_batch_size} global batch size x {gradient_accumulation_steps} accumulation")
    ...
    accumulate_steps = 0  # Track accumulation step count

    while True:
        for x, y in loader:
            # Prepare inputs for accelerator
            ...
            if 'cos_loss' in loss_dict:
                running_loss += (mse_loss.item() * gradient_accumulation_steps)  # Multiply by accumulation steps to get true loss
            else:
            ...
            accumulate_steps += 1
            if accumulate_steps >= gradient_accumulation_steps:
                max_grad_norm = train_config['optimizer'].get('max_grad_norm', None)
                if max_grad_norm is not None and accelerator.sync_gradients:
                    # Clip gradients
                    accelerator.clip_grad_norm_(model.module.parameters(), max_grad_norm)
                # Optimizer step
                opt.step()
                opt.zero_grad()
                # EMA update
                update_ema(ema, model.module)
                accumulate_steps = 0  # Reset accumulation step count
                log_steps += 1
                train_steps += 1
                ...
                # Logging and checkpoint saving logic remains unchanged
                if train_steps % train_config['train']['log_every'] == 0:
                    ...
                    avg_loss = torch.tensor(running_loss / (log_steps * gradient_accumulation_steps), device=device)
...
if __name__ == "__main__":
    ...
    accelerator = Accelerator(
        gradient_accumulation_steps=train_config['train']['gradient_accumulation_steps']
    )
